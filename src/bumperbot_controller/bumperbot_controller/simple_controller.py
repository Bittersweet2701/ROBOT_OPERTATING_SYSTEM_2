...
class SimpleController(Node):
    def __init__(self):
        ...
        self.get_logger().info("Using_wheel_radius %f" % self.wheel_radius_)
        self.get_logger().info("Using_wheel_separation %f" % self.wheel_separation_)
        ...
        self.wheel_velocity_sub_ = self.create_subscription(TwistStamped, "bumperbot_controller/cmd_vel", self.velCallback, 10)
        self.joint_sub_ = self.create_subscription(JointState, "joint_states", self.jointCallback, 10)

        self.speed_conversion_ = np.array([[self.wheel_radius_/2, self.wheel_radius_/2],
                                          [self.wheel_radius_/self.wheel_separation_, -self.wheel_radius_/self.wheel_separation_]])
        self.get_logger().info("Speed_Conversion matrix is %s" % self.speed_conversion_)
    ...
    def jointCallback(self, msg):
        ...
        self.get_logger().info("Linear_speed: %f, Angular_speed: %f" % (linear, angular))
