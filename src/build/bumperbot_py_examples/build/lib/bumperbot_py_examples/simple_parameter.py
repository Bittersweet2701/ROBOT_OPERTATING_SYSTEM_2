    ...
    def paramChangeCallback(self, params):
        ...
        for param in params:
            if param.name == "simple_int_param" and param.type_ == Parameter.Type.INTEGER:
                self.get_logger().infor("Param simple_int param change! New value is %d" % param.value)
                result.successful = True

            if param.name == "simple_string_param" and param.type == Parameter.Type.STRING:
                self.get_logger().info("Param simple_string_param change! New value is %d" % param.value)
                result.successful = True
